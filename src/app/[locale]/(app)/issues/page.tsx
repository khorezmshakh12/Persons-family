...
export default async function IssuesPage() {
  ...
  // Filtered at the DB level so the query stays bounded as resolved issues
  // accumulate over months instead of fetching every issue ever every load.
  ...
  const [{ data: issuesData }, { data: assignees }] = await Promise.all([
    supabase
      .from('issues')
      .select(
      ...
      .or(`status.neq.done,resolved_at.is.null,resolved_at.gte.${sevenDaysAgo}`)
