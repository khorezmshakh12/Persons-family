        ...
        <div className="flex flex-wrap items-center gap-2 border-t border-white/10 pt-4">
          <button
            ...
            className={cn(
              ...
              activeTab === 'shop' ? 'text-white' : 'text-white/70 hover:text-white hover:bg-white/5',
            )}
          >
            {activeTab === 'shop' && (
              <motion.div
                layoutId="activeMarketTab"
                className="absolute inset-0 rounded-lg bg-white/20 border border-white/20 shadow-sm"
                transition={springs.snappy}
              />
            )}
            <ShoppingBag className="relative z-10 size-4" />
            <span className="relative z-10">{t('tabs.shop')}</span>
            <span className="relative z-10 rounded-full bg-white/15 px-2 py-0.5 text-xs">
              {items.length}
            </span>
          </button>
          ...
          <button
            ...
            className={cn(
              'relative flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-colors',
              activeTab === 'myOrders' ? 'text-white' : 'text-white/70 hover:text-white hover:bg-white/5',
            )}
          >
            {activeTab === 'myOrders' && (
              <motion.div
            ...
            <Package className="relative z-10 size-4" />
            <span className="relative z-10">{t('tabs.myOrders')}</span>
            {orders.length > 0 && (
              <span className="relative z-10 rounded-full bg-white/15 px-2 py-0.5 text-xs">
                {orders.length}
              </span>
          ...
          {adminView.allowed && (
            ...
            >
              {activeTab === 'admin' && (
                <motion.div
                  layoutId="activeMarketTab"
                  className="absolute inset-0 rounded-lg bg-amber-500/20 border border-amber-500/30 shadow-sm"
                  transition={springs.snappy}
                />
              )}
              <ShieldCheck className="relative z-10 size-4" />
              <span className="relative z-10">{t('tabs.admin')}</span>
              {adminView.pendingOrders.length > 0 && (
                <span className="relative z-10 rounded-full bg-red-500/80 text-white px-2 py-0.5 text-xs font-bold">
