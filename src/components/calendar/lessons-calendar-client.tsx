  ...
  return (
    <div className={cn(GLASS_CARD, 'flex flex-col gap-4 p-6')}>
      <div className="flex items-center justify-between gap-2">
        <h2 className="font-heading text-lg font-semibold text-au-ink">{t('monthTitle')}</h2>
          ...
          return (
            <button
              ...
              className={cn(
                ...
                isSelected ? 'border-au-faint bg-au-card-2' : 'border-au-line bg-au-card-2 hover:bg-au-card-2',
                day === today && !isSelected && 'ring-2 ring-au-faint',
              )}
            >
              <span className="text-xs font-medium text-au-muted">{day}</span>
              {events.length > 0 && (
      ...
      {!hasAnyLessons && <p className="text-xs text-au-muted">{t('noLessonsThisMonth')}</p>}
      ...
      {selectedDay && (
        <div className="flex flex-col gap-2 border-t border-au-line pt-4">
          ...
          {selectedEvents.length === 0 ? (
            <p className="text-xs text-au-muted">{t('noLessonsThisDay')}</p>
          ) : (
            <div className="flex flex-col gap-2">
              {selectedEvents.map((event) => (
                <Link
                  ...
                  className="flex flex-col gap-0.5 rounded-lg border border-au-line bg-au-card-2 px-3 py-2 transition-colors hover:bg-au-card-2"
                >
                  <span className="text-sm font-medium text-au-ink">{event.groupName}</span>
                  {event.teacherName && <span className="text-xs text-au-muted">{event.teacherName}</span>}
                  {event.topic && <span className="text-xs text-au-muted">{event.topic}</span>}
                </Link>
              ))}
            </div>
          )}
